...
import { deleteshop, getallshops, getusershops } from "../../../redux/actions/shopactions";
...
export default function ShopAdmin() {
  ...
  const columns = useMemo(
          ...
          return (
           <Stack direction="row" spacing={1} sx={{ alignItems: "center" }}>
              ...
              <Typography
                sx={{ fontSize: "15px", textTransform: "capitalize" }}
              >
                No of Products{params?.row?.products?.length}
              </Typography>
              </Box>
            </Stack>
          );
      ...
      },
   {
        ...
        width: 150,
        renderCell: (params) => {
          return <Stack><Typography>{params?.row?.owner?.username}</Typography><Typography>{params?.row?.shopphone}</Typography></Stack>;
        },
      },
