...
exports.signup = async (req,res) =>{
    try {
        ...
        if (mail){
            return res.status(200).json({success:true,message:"Email has been sent to your email accout please verify to continue",user:userdata._id})}
        } catch (error) {
...
exports.verifyuser = async (req,res)=>{
    ...
    try {
        ...
        if (fuser.verifyemailtoken == otp) {
            console.log(fuser)
            const user = await User.findByIdAndUpdate({_id:id},{isverifiedemail:true,verifyemailtoken:null},{new:true})
            return res.status(200).json({success:true,message:"Register Success Please Login to Continue",user})
        }

        return res.status(404).json({success:false,message:"Invalid Attempt!"})
    ...
    try {
        ...
        if (user) {
            ...
            if (compare) {
                ...
                return res.status(200).json({success:true,message:"Login Success",data:user,token})
            }
            else{
            ...
            return res.status(400).json({success:false,message:"Invalid Email Please register to continue"})
        }
    } catch (error) {
        return res.status(400).json({success:false,message:error.message})
    }
...
exports.passwordchange = async (req,res)=>{
    try {
        ...
        if (check) {
            ...
            const nuser = await User.findByIdAndUpdate({_id:user._id},{password:p},{new:true})
